...
14. 観測が朝の短時間スナップショットなら、日全体の総括ではなく**その時点の流れ**として narrative を組み立てる
...
- 抽象評価語ではなく、成果・変更・判断を直接書く
- 裏付けのない未来文脈や意図推定は書かない
...
- 箇条書きメモの焼き直しにしない。各セクションは**最低 1 つの完結した段落**で返す
- `今日の中心` は、可能なら 2 段落以上で **主作業 → 転換点 / 判断 → 結果** の流れを見せる
...
- 冒頭の 1-2 文で、**今日何を変えて何が分かったか**を先に出す
- 冒頭で「作業していた」「走らせていた」だけを述べて終わらせず、**何が進んだか / 何が見えてきたか**まで出す
- `背景` は 1-2 文まで。主題がすでに明確な日は、省略または極小化してよい
- `今日の中心` の第1段落で、主作業とその結果を先に述べる
- `背景` / `今日の中心` / `気づき` の各見出しの中を 1 行メモだけで終わらせない
- Chrome 履歴や単独の low confidence イベントは主題や転換点にしない。入れる場合も補助的な注記に留める
- 朝の短時間観測しかない日は、「今日は〜した日」と言い切らず、「朝時点では〜を進めていた」などスナップショットとして表現する
- テストや実装の進め方を推測で断定しない
  - **NG:** `テストを先に書くことで境界が明確になった`
  - **OK:** `テスト追加を通じて、入出力の境界が明確になりました`
  - **OK:** `テスト追加により、どの入出力を固定すべきかが見えました`
