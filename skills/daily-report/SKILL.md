...
- 先頭は必ず `## 日報 YYYY-MM-DD`
- 活動項目は 3-6 個
- 自分用の活動は `### 今日の流れ` の下に番号付きリストで並べる（トピック名の個別 `###` 見出しにしない）
- 共有用の活動見出しは `### 実装` / `### 調査（任意）` / `### 設計 / 判断（任意）` のカテゴリ名を使う
  トピック名（`### ハッカソン発表準備` 等）の個別見出しにしない
- 根拠の扱いは mode で異なる:
...
- 自分用の各項目は、**何が進んだか / 何を判断したか**を先に書き、観測した source の羅列だけで終わらせない
- 自分用の各項目は、`[活動] → [結果 or 判断] → 根拠` の順に読めるようにする
- browser 履歴や low confidence の補助情報は、単独で番号付き主項目にせず、既存項目の注記か `備考` に寄せる
- browser 履歴だけを根拠にする場合は、成果や意思決定の主語にせず「補助推定」と明示する
- 朝だけの短い観測窓で全体像が薄い場合は、「今日は〜した日だった」と日全体を断定せず、「朝時点では〜が動いていた」と書く
- 根拠は短文化する
...
- 自分用日報が `### 今日の流れ` + 番号リスト形式になっている
- 共有用日報が `### 今日の概要` → カテゴリ見出し（実装/調査等）→ `### 未完了の手がかり` → `### 参考: 根拠一覧` の構造になっている
